...
# Load the IR module before finalizer/validation imports can initialize the
# readiness evaluator registry. This keeps the facade import-safe while the
# sibling gate migration is dirty or partially imported.
from ... import protocol_ir as _protocol_ir  # noqa: F401
...
from . import GenerationStageRunner, GenerationState, LoadContextStage
...
def _protocol_delivery_folder_complete(
    ...
    *,
    ...
    required = [
        ...
        source_dir / "request.spec.yaml",
        source_dir / "protocol.ir.json",
    ...
    if require_final_reports:
        required.extend(
            [
                source_dir / "generation_manifest.json",
                source_dir / "GENERATION_WORKFLOW.md",
            ]
        )
...
def render_generation_summary(manifest: dict[str, Any]) -> str:
    ...
    if manifest.get("ready_to_import") and (readiness.get("script_editor_load") or {}).get("status") != "passed":
        ...
        lines.extend(
            [
                ...
                "> "
                + (
                    "IMPORT READY, SCRIPT EDITOR LOAD FAILED. The bundle passed offline import gates, but the optional "
                    "FluentControl import/load diagnostic reported a load failure. Resolve the Script Editor "
                    "load error before continuing. Hardware use still requires operator review."
                    if load_status in {"failed", "load_failed"}
                    else "IMPORT READY, SCRIPT EDITOR LOAD NEEDS REVIEW. Review the Gate 27 or manual Script Editor "
                    "handoff details and record a definitive load result before continuing. Hardware use still "
                    "requires operator review."
                    if load_status == "needs_review"
                    else "IMPORT READY, SCRIPT EDITOR LOAD NOT VERIFIED. The bundle passed offline import gates, but "
                    "Script Editor load/open is a separate check. Run the optional FluentControl "
                    "import/load diagnostic or manually open the generated method and record the result. "
                    "Hardware use still requires operator review."
                ),
            ]
        )
