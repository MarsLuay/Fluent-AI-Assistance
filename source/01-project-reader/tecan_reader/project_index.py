...
def _index_references(
    ...
    for ref in script.get("references", []):
        ...
        _insert_entity(
            conn,
            EntityRecord(zeia_id, script_id, kind, object_name, type_id, source_path, metadata),
        )
        if kind != "reference":
            _insert_entity(
                conn,
...
def _index_object(conn: sqlite3.Connection, zeia_id: int, obj: dict[str, Any]) -> None:
    ...
    for name in obj.get("names", []):
        _insert_entity(
            conn,
            EntityRecord(zeia_id, None, object_kind, name, "object_name", source_path, metadata),
        )
...
def _count(conn: sqlite3.Connection, table: str) -> int:
    ...
    # Validate table name against a schema-derived allowlist
    valid_tables = {
