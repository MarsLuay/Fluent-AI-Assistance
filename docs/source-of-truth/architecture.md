...
Three operator surfaces share one Python API: **CLI**, **MCP**, and direct **Python** imports. Application services derive one `authoring_status` contract; CLI renders it and MCP forwards it unchanged. Sibling packages (`01-project-reader`, `02-worklist-builder`, `00-shared`, `source/tools`) feed inspect/index/GWL/simulator assets into that pipeline.
...
| generation | Request specs, canonical package-owned workflow, repair, application services, shared authoring status |
| protocol-ir | IR load/normalize/schema |
| api-v2 | Live/offline FluentControl command + runtime surfaces |
| gates-validation | Registry-addressable package evaluators; validation orchestration and compatibility facades |
...
Published bundle roots contain the ZEIA, `run_tecan_bundle_setup.bat`,
`RECREATE_SCRIPT.md`, `media/`, and `source/` (plus optional
`RECIPE_GROUP_NOTES.md`). The helpers, manifests, request spec, IR, metadata,
generated Python, and reports are nested under `source/`; runtime diagnostics
may add `temp_files/`.
...
4. **Validate / gates** → all 27 active implementations dispatch through the readiness registry before package; Gate 27 remains optional/live.
